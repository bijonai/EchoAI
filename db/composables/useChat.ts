...
import type { AgentMessage, ChatMessage, UserMessage } from "~/types/message";
...
export function useChat(db: NodePgDatabase, params: UseChatParams) {
  ...
  const auth = and(eq(chats.uid, userId), eq(chats.id, chatId));
  ...
  async function pull(selector: Record<string, PgColumn>) {
    const column = single(
      await db.select(selector).from(chats).where(auth).limit(1)
    );
  ...
  function addMessageChunk(type: "user" | "agent", chunk: string) {
    if (!messages) return;
    if (messages[messages.length - 1].type === type) {
      (messages[messages.length - 1] as UserMessage | AgentMessage).content +=
        chunk;
    } else {
      messages.push({
        type,
